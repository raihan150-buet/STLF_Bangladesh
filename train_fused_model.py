...
def train_fused(config_path, data_path, quantum_feature_path, checkpoint_path, saved_model_path, resume_checkpoint_path=None):
    ...
    file_config.update({
        'data_path': data_path, 'quantum_feature_path': quantum_feature_path,
        'checkpoint_path': checkpoint_path, 'saved_model_path': saved_model_path
    })
    
    active_config = file_config.copy()
    
    if active_config.get("use_wandb", False):
        run_tags = ["quantum-enhanced"] if active_config.get('use_quantum_features', False) else ["classical-benchmark"]
        run_name = f"{active_config.get('model_type', 'model')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        try:
            wandb.init(project=active_config.get("wandb_project"), entity=active_config.get("wandb_entity"), config=active_config, name=run_name, tags=run_tags, reinit=True)
