...
def load_checkpoint(checkpoint_path, device):
    ...
    return {
        'model': model,
        'optimizer': optimizer,
        'scheduler': scheduler, # <-- ADDED: Return the scheduler
...
def train(config_source, data_path, checkpoint_path, saved_model_path, resume_checkpoint_path=None):
    ...
    # --- Initialize or resume model, optimizer, AND scheduler ---
    ...
    for epoch in range(start_epoch, active_config["num_epochs"]):
        ...
        # It monitors the validation loss and adjusts LR if needed
        scheduler.step(epoch_val_loss) 
        
        if active_config.get("use_wandb") and wandb.run:
            # Log the current learning rate to W&B
        ...
        if is_best:
            ...
            epochs_no_improve = 0
            # Pass the scheduler to save_checkpoint
