...
# Assuming these imports point to the correct files in your project
from utils.preprocessing import load_and_preprocess_data
from utils.data_loader import TimeSeriesDataset
from models import get_model

def save_checkpoint(epoch, model, optimizer, config, best_val_loss, epochs_no_improve, is_best=False, current_run_id=None):
    """Saves model checkpoint to the appropriate directory."""
    checkpoint_dir = config.get("checkpoint_path", "checkpoints")
    
    # Determine save directory: best models go to 'saved_model_path', others to 'checkpoint_path'
    if is_best:
...
def train(config_source, data_path, checkpoint_path, saved_model_path, resume_checkpoint_path=None):
    ...
    # Inject runtime paths into the config
    ...
    # Update config with dynamic input_size from preprocessing
    ...
    if resume_checkpoint_path:
        ...
        model, optimizer = checkpoint_data['model'].to(device), checkpoint_data['optimizer']
    ...
    for epoch in range(start_epoch, active_config["num_epochs"]):
        ...
        if active_config.get("use_wandb") and wandb.run:
            wandb.log({"epoch": epoch + 1, "train_loss": epoch_train_loss, "val_loss": epoch_val_loss})
        ...
        if is_best:
            ...
            epochs_no_improve = 0
            save_checkpoint(epoch, model, optimizer, active_config, best_val_loss, epochs_no_improve, is_best=True, current_run_id=current_wandb_run_id)
        else:
            epochs_no_improve += 1

        if (epoch + 1) % active_config.get("checkpoint_freq", 5) == 0 and not is_best:
            save_checkpoint(epoch, model, optimizer, active_config, best_val_loss, epochs_no_improve, is_best=False, current_run_id=current_wandb_run_id)
